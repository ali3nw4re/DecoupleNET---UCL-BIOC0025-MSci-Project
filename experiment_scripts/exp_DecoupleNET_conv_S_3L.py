...
from tensorflow.keras.layers import Input, Concatenate, Conv1D, Reshape
...
y_output = y_output.reshape((-1, int(512), 2))
...
hidden2 = Conv1D(filters=filters_number, kernel_size=7, activation="relu", padding="same")(hidden1)
hidden3 = Conv1D(filters=filters_number, kernel_size=10, activation="relu", padding="same")(hidden2)

output_layer = Conv1D(filters=2, kernel_size=8, activation="linear", padding="same")(hidden3)
#Model architecture^
