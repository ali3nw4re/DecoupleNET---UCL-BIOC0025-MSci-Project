...
class minuk_gen_FID:
    ...
    def __init__(self):
        ...
        #As all of the models were trained on spectra randing from 1-5 peaks, 
        # it is reccomended that the length of the arrays does not exceed 5.
...
fig.canvas.manager.set_window_title("1D Train prediction")
...
fig.canvas.manager.set_window_title("DecoupleNET_conv prediction")
axs[0][0].plot(np.real(ppm), np.real(spectrum2))
axs[0][0].set_title("Coupled (Input)")
axs[0][0].set_xlabel("13C PPM")
axs[0][1].plot(np.real(ppm), np.real(spectrum3))
